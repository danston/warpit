...
namespace warpit {
    ...
    struct event {
      ...
      VOIDT *eventptr;      /* Can be a vertex or the location of a circle event. */
    ...
    #ifdef ANSI_DECLARATORS
    VOIDT *trimalloc(int size)
    #else /* not ANSI_DECLARATORS */
    VOIDT *trimalloc(size)
    int size;
    #endif /* not ANSI_DECLARATORS */

    {
      ...
      memptr = (VOIDT *) malloc((unsigned int) size);
      if (memptr == (VOIDT *) NULL) {
    ...
    */
    ...
    {
      pool->firstblock = (VOIDT **) NULL;
      pool->nowblock = (VOIDT **) NULL;
      pool->nextitem = (VOIDT *) NULL;
      pool->deaditemstack = (VOIDT *) NULL;
      pool->pathblock = (VOIDT **) NULL;
      pool->pathitem = (VOIDT *) NULL;
      pool->alignbytes = 0;
      pool->itembytes = 0;
    ...
    if (alignment > (signed) sizeof(VOIDT *)) {
        pool->alignbytes = alignment;
      } else {
        pool->alignbytes = sizeof(VOIDT *);
      }
      ...
      pool->firstblock = (VOIDT **)
        trimalloc(pool->itemsfirstblock * pool->itembytes + (int) sizeof(VOIDT *) +
                  pool->alignbytes);
    ...
    {
      while (pool->firstblock != (VOIDT **) NULL) {
        pool->nowblock = (VOIDT **) *(pool->firstblock);
        trifree((VOIDT *) pool->firstblock);
    ...
    #ifdef ANSI_DECLARATORS
    VOIDT *poolalloc(struct memorypool *pool)
    #else /* not ANSI_DECLARATORS */
    VOIDT *poolalloc(pool)
    struct memorypool *pool;
    #endif /* not ANSI_DECLARATORS */

    {
      ...
      if (pool->deaditemstack != (VOIDT *) NULL) {
        newitem = pool->deaditemstack;               /* Take first item in list. */
        pool->deaditemstack = * (VOIDT **) pool->deaditemstack;
      } else {
        /* Check if there are any free items left in the current block. */
        if (pool->unallocateditems == 0) {
          /* Check if another block must be allocated. */
          if (*(pool->nowblock) == (VOIDT *) NULL) {
            /* Allocate a new block of items, pointed to by the previous block. */
            newblock = (VOIDT **) trimalloc(pool->itemsperblock * pool->itembytes +
                                           (int) sizeof(VOIDT *) +
                                           pool->alignbytes);
            *(pool->nowblock) = (VOIDT *) newblock;
            /* The next block pointer is NULL. */
            *newblock = (VOIDT *) NULL;
          }

          /* Move to the new block. */
          pool->nowblock = (VOIDT **) *(pool->nowblock);
        ...
        pool->nextitem = (VOIDT *) ((char *) pool->nextitem + pool->itembytes);
    ...
    #ifdef ANSI_DECLARATORS
    void pooldealloc(struct memorypool *pool, VOIDT *dyingitem)
    #else /* not ANSI_DECLARATORS */
...

    {
      ...
      *((VOIDT **) dyingitem) = pool->deaditemstack;
    ...
    #ifdef ANSI_DECLARATORS
    VOIDT *traverse(struct memorypool *pool)
    #else /* not ANSI_DECLARATORS */
    VOIDT *traverse(pool)
    struct memorypool *pool;
    #endif /* not ANSI_DECLARATORS */

    {
      ...
      if (pool->pathitemsleft == 0) {
        /* Find the next block. */
        pool->pathblock = (VOIDT **) *(pool->pathblock);
        /* Find the first item in the block.  Increment by the size of (VOID *). */
        alignptr = (unsigned long) (pool->pathblock + 1);
        /* Align with item on an `alignbytes'-byte boundary. */
        pool->pathitem = (VOIDT *)
      ...
      pool->pathitem = (VOIDT *) ((char *) pool->pathitem + pool->itembytes);
      pool->pathitemsleft--;
      return newitem;
    }
...

    {
      ...
      pooldealloc(&m->triangles, (VOIDT *) dyingtriangle);
    }
...

    {
      ...
      pooldealloc(&m->subsegs, (VOIDT *) dyingsubseg);
    }
...

    {
      ...
      pooldealloc(&m->vertices, (VOIDT *) dyingvertex);
    }
...

    {
      ...
      pooldealloc(&m->badsubsegs, (VOIDT *) dyingseg);
    }
...

    {
      ...
      trifree((VOIDT *) m->dummytribase);
      if (b->usesegments) {
        pooldeinit(&m->subsegs);
        trifree((VOIDT *) m->dummysubbase);
      }
    ...
    */
...

    {
      ...
      trifree((VOIDT *) m->infvertex1);  /* Deallocate the bounding box vertices. */
      trifree((VOIDT *) m->infvertex2);
      trifree((VOIDT *) m->infvertex3);
...

    {
      ...
      *events = (struct event *) trimalloc(maxevents * (int) sizeof(struct event));
      ...
      for (i = 0; i < m->invertices; i++) {
        ...
        (*events)[i].eventptr = (VOIDT *) thisvertex;
      ...
      *freeevents = (struct event *) NULL;
      for (i = maxevents - 1; i >= m->invertices; i--) {
        (*events)[i].eventptr = (VOIDT *) *freeevents;
        *freeevents = *events + i;
      }
    }
...

    {
      ...
      if (eventvertex != (vertex) NULL) {
        ...
        deadevent->eventptr = (VOIDT *) *freeevents;
...

    {
      ...
      if (checkvertex == splaytree->keydest) {
        ...
        pooldealloc(&m->splaynodes, (VOIDT *) splaytree);
...

    {
      ...
      eventheap[0]->eventptr = (VOIDT *) freeevents;
    ...
    */
        ...
        nextevent->eventptr = (VOIDT *) freeevents;
        ...
        if (check4events) {
          ...
          if (lefttest > 0.0) {
            ...
            newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                       lefttest);
            newevent->eventptr = (VOIDT *) encode(lefttri);
          ...
          if (righttest > 0.0) {
            ...
            newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                       righttest);
            newevent->eventptr = (VOIDT *) encode(farrighttri);
            eventheapinsert(eventheap, heapsize, newevent);
            heapsize++;
      ...
      if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
        ...
        while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
          ...
          if (m->badsubsegs.items > 0) {
            ...
            pooldealloc(&m->badtriangles, (VOIDT *) badtri);
          }
        }
      }
...

    {
      ...
      m->vertices.deaditemstack = (VOIDT *) NULL;
